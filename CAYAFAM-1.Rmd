...
# no se hizo porque no tenemos las concentraciones de los contaminantes 
...
Fig.A_model_PAS <- ggplot2::ggplot(data = PAS_coef, mapping =aes( x= estimate, y = exposure)) + 
  ...
  xlab("Beta estimates") + ylab("Exposures")  + theme_minimal(base_size = 12) + xlim(-15, 15)  + theme(plot.title = element_text (face='bold')) + ggtitle("A) Diastolic BP") 


ggplot2::ggplot(data = PAS_coef, mapping = aes(x = exposure , y = estimate, ymin = conf.low, ymax = conf.high)) + geom_pointrange() + geom_hline(yintercept = 0, linetype = "dashed", color = "red") + coord_flip() +  ylab("Beta estimates CI95%") + xlab("Exposures") + scale_y_continuous(limits  = c(-15, 15))  + theme(plot.title = element_text (face='bold')) + ggtitle("A) Diastolic BP") + theme_minimal()
...
Fig.B_model_PAS  <- ggplot2::ggplot(data = PAD_coef, mapping =aes( x= estimate, y = exposure)) + 
  ...
  geom_pointrange(aes(xmin = conf.low, xmax = conf.high), shape = 16) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "red") + 
  xlab("Beta estimates") + ylab("Exposures")  + theme_minimal(base_size = 12) + xlim(-15, 15)  + theme(plot.title = element_text (face='bold')) + ggtitle("B) Systolic BP") 
...
# wee group all the figures for each of the models in one figure ussing the library patchwork. 

fig1 <- ((Fig.A_model_PAS / Fig.B_model_PAS) | Fig.C_model_SintResp / plot_spacer() ) + 
  plot_annotation(tag_levels = "A")

col1 <- (Fig.A_model_PAS / Fig.B_model_PAS) + plot_layout(heights = c(1, 1))
