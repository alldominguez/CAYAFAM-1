...
Fig.C_model_SintResp <- ggplot2::ggplot(data = EnfResp_OR, mapping =aes( x= estimate, y = exposure)) + 
  ...
  xlab("OR adjusted") + ylab("Exposures")  + theme_minimal() + theme(plot.title = element_text (face='bold')) + ggtitle("C) Respiratory Symptoms") 
...
Fig.A_model_PAS <- ggplot2::ggplot(data = PAS_coef, mapping =aes( x= estimate, y = exposure)) + 
  ...
  xlab("Beta estimates") + ylab("Exposures")  + theme_minimal() + xlim(-15, 15)  + theme(plot.title = element_text (face='bold')) + ggtitle("A) Diastolic BP")
...
Fig.B_model_PAS  <- ggplot2::ggplot(data = PAD_coef, mapping =aes( x= estimate, y = exposure)) + 
  ...
  geom_pointrange(aes(xmin = conf.low, xmax = conf.high), shape = 16) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "red") + 
  xlab("Beta estimates") + ylab("Exposures")  + theme_minimal() + xlim(-15, 15)  + theme(plot.title = element_text (face='bold')) + ggtitle("B) Systolic BP")
...
col2 <- (Fig.C_model_SintResp / plot_spacer()) + plot_layout(heights = c(0.5, 0.5))

col1|col2

ggsave("fig_1_models.tiff", width = 6, height = 6) 
