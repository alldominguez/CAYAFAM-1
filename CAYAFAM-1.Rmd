...
PAS_coef <- data.frame(
  term = c("High Exposure", "Physical Condition (Regular)", "Physical Condition  (Optimal)" ),
  estimate = c(6.77, 2.34, 0.17),
  conf.low = c(1.04, -8.11, -11.53),
  conf.high = c(12.5, 12.79, 11.88))

PAS_coef$term <- factor(PAS_coef$term, PAS_coef$term)

library(GGally) 
Fig.A_model_PAS <- ggcoef(PAS_coef, exponentiate = FALSE, vline_color = "red", vline_linetype = "dashed") + xlab("Beta estimates") + ylab("Exposure") + theme_minimal() + ggtitle("A) Systolic Blood Pressure") + xlim(-15, 15)
...
PAD_coef$term <- factor(PAD_coef$term, PAD_coef$term)

library(GGally) 
Fig.B_model_PAS <- ggcoef(PAD_coef, exponentiate = FALSE, vline_color = "red", vline_linetype = "dashed") + xlab("Beta estimates") + ylab("Exposure") + theme_minimal() + ggtitle("B) Diastolic Blood Pressure") + xlim(-15, 15)


Fig.A_model_PAS + Fig.B_model_PAS + Fig.C_model_SintResp
...
library(viridis)
corplot <- ggplot(data =melted_cormat, mapping = aes(x= Var1 , y =Var2, fill=value)) + geom_tile() + scale_fill_viridis(name = "Spearman\nCorrelation", limit = c(0,1)) + #revisamos el cormat para poner los limites
 xlab("") + ylab("") + coord_fixed()+ theme_light()
